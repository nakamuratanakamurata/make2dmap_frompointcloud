...
def create_image(coordinates, config):
    ...
    threshold_count = config['colors']['threshold_count']
    below_threshold_color = tuple(config['colors']['below_threshold_color'])
    above_threshold_color = tuple(config['colors']['above_threshold_color'])
    ...
    above_threshold = sum(1 for count in pixel_counts.values() if count >= threshold_count)
    print(f"Pixels above threshold ({threshold_count}): {above_threshold}")
...
def main():
    ...
    # 出力ファイル名を設定ファイルから取得
    output_file_name = config['output_files']['point2png_over_threshold_output']
